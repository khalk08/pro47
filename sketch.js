//task for today: add food, a food count and the score in the end
...
var banana, bananaImg, water, waterImg, sandwich, sandwichImg;
var foodGroup;
var diamondsGroup;
var obstaclesGroup;
var score=0;
...
function preload(){
   ...
   demon1Img = loadImage("images/demon1.gif");
   ...
   bananaImg = loadImage("images/banana.gif");
   waterImg = loadImage("images/water.gif");
   sandwichImg = loadImage("images/sandwich.gif");
}
...
function setup(){
  ...
  banana = createSprite(windowWidth/2,Math.round(random(200, 400)), 20,20 );
  banana.addImage("banana", bananaImg);
  banana.visible = false;
  banana.velocityX = -4;
  banana.scale = 0.1;

  water = createSprite(windowWidth/2,Math.round(random(200, 400)), 20,20 );
  water.addImage("water", waterImg);
  water.visible = false;
  water.velocityX = -4;
  water.scale = 0.1;

  sandwich = createSprite(windowWidth/2,Math.round(random(100, 300)), 20,20 );
  sandwich.addImage("sandwich", sandwichImg);
  sandwich.visible = false;
  sandwich.velocityX = -4;
  sandwich.scale = 0.1;
...
function draw(){
  ...
  textSize(20);
  fill("white");
  text("Score: " +  score, windowWidth-200, 25);
  textSize(20);
  fill("white");
  text("Food Count: " +  foodCount, windowWidth-400, 25);
...
function spawnFoods(){
  if(frameCount % 90 === 0) {
    food = createSprite(windowWidth/2,Math.round(random(200, 400)), 20,20);
    food.debug = true;
    food.velocityX = -4;
    var rand = Math.round(random(1,3));
     switch(rand) {
      case 1: food.addImage(bananaImg);
              break;
      case 2: food.addImage(waterImg);
              break;
      case 3: food.addImage(sandwichImg);
    default: break;
  }
   food.scale = 0.3;
   food.lifetime = 300;
